...
from pers_cohort_query.integrate import get_density_peak, load_tabular_data, load_query_inputs, get_all_density_peak, get_pers_cohort_density_peaks, compute_cohort_shifts, write_output
import pandas as pd
import pytest
...
def test_load_tabular_data_raises_error_for_invalid_date_format(tmp_path, invalid_date, expected_message):
    file = tmp_path / "test.csv"
    file.write_text(
        f"col1,col2\n1,2020-01-01\n2,{invalid_date}"
    )
...
def test_load_query_inputs_raises_error_for_ids_in_cohorts_not_in_lab_values(tmp_path):
    ...
    with pytest.raises(ValueError, match="Individuals in cohorts not found in lab values"):
        load_query_inputs(lab_values_file, cohorts_file)
...
def test_load_query_inputs_raises_error_for_ids_in_lab_values_not_in_cohorts(tmp_path):
    ...
    with pytest.raises(ValueError, match="Individuals in lab values not found in cohorts"):
        load_query_inputs(lab_values_file, cohorts_file)
